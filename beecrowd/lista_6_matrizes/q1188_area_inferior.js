...
function soma_elementos_matriz(matriz){
    ...
    for (let i = 0; i < matriz.length; i++) {
        for (let j = 0; j < matriz.length; j++) {
            if(abaixo_diagonal_principal(i, j) && abaixo_diagonal_secundaria(i, j, ordem)){
...
const abaixo_diagonal_secundaria = (i, j, ordem) => {return i + j > ordem} 
const abaixo_diagonal_principal = (i, j) => {return j < i} 
