...
function main(){
    ...
    if(operacao === 'S'){
        const soma = soma_linha_matriz(matriz, index)
        console.log(soma.toFixed(1))
    } else if (operacao === 'M'){
        const media = media_linha_matriz(matriz, index)
...
function soma_linha_matriz(matriz, linha){
    let soma = 0

    for (let index = 0; index < matriz.length; index++) {
        soma += Number(matriz[linha][index])
    }

    return soma
}

function media_linha_matriz(matriz, linha){
    let soma = 0
